 ...
 */
public final class GoogleOAuthAction extends Action {
	private static boolean debug = true;


	/**  Constructor for the GoogleOAuthAction object */
...
/* 		String requestedUrl = req.getRequestURI();
		...
		try {
			...
			if (command == null || command.trim().length() == 0) {
				...
				return doGoogleProxy (googleAuthHelper, mapping, form, req, response);
			}
			else if (command.equals("client")) {
				return doGoogleClient (googleAuthHelper, mapping, form, req, response);
			}
			else if (command.equals("upload")) {
				return doGoogleUpload (googleAuthHelper, mapping, form, req, response);
			}
			else if (command.equals("token")) {
				return doGoogleRefreshToken (googleAuthHelper, mapping, form, req, response);
			}
	...
	private ActionForward doGoogleUpload (GoogleAuthHelper googleAuthHelper,
										 ActionMapping mapping,
										 ActionForm form,
										 HttpServletRequest req,
										 HttpServletResponse response) {
		...
		String apiData = null;
		try {
			apiData = GoogleAPIRequestor.upload(apiUrl, postBody, metadata);
	...
	/**
	EXPERIMENTAL - here is where we might ensure that a GoogleClientBean
	is in the session before forwarding to client.response.<p>
	This method is currently not USED
	*/
	private ActionForward doGoogleClient (GoogleAuthHelper googleAuthHelper,
										 ActionMapping mapping,
										 ActionForm form,
										 HttpServletRequest req,
										 HttpServletResponse response) {
		...
		prtln ("returning client.response ...");
		return mapping.findForward ("client.response");
...
/* 			prtln ("resp: " + resp);
	...
	private ActionForward doGoogleAuth (GoogleAuthHelper googleAuthHelper,
		...
		try {
			if ((req.getParameter("code") == null || req.getParameter("state") == null) && req.getParameter("error") == null) {
				...
				String googleAuthLoginUrl = googleAuthHelper.buildLoginUrl(req.getParameter("context"));
				prtln ("redirecting to googleAuthLoginUrl: " + googleAuthLoginUrl);
				
				// redirect to login url - context is sent in param
				return new ActionForward (googleAuthLoginUrl, true);
				...
				*/
				...
				if (req.getParameter("code") != null && state != null
					...
					*/  
					...
					// String userInfoJson = googleAuthHelper.getUserInfoJson(req.getParameter("code"));
					// GoogleUserInfo googleUserInfo = googleAuthHelper.getGoogleUserInfo();
			...
			prtln("GoogleOAuthAction caught exception.");
			e.printStackTrace();
			return mapping.findForward("auth.page");
		} catch (Throwable e) {
			prtln("GoogleOAuthAction caught exception: " + e);
			e.printStackTrace();
			return mapping.findForward("auth.page");
		}
	}
	...
	private void showParams(HttpServletRequest req) {
		...
		while (params.hasMoreElements()) {
			...
			if (paramName.equals("command") || paramName.equals("apiUrl")) {
	...
	private final void prtlnErr(String s) {
		System.err.println(getDateStamp() + " Error GoogleOAuthAction: " + s);
	}
	...
	private final void prtln(String s) {
		if (debug) {
			// System.out.println(getDateStamp() + " GoogleOAuthAction: " + s);
			System.out.println("GoogleOAuthAction: " + s);
		}
	}
